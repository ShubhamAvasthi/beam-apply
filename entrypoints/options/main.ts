...
const willingToRelocateInput = document.querySelector<HTMLSelectElement>(
  "#willing-to-relocate",
)!;
const howDidYouHearInput =
  document.querySelector<HTMLInputElement>("#how-did-you-hear")!;
...
const resumeRemove =
  document.querySelector<HTMLButtonElement>("#resume-remove")!;
const status = document.querySelector<HTMLSpanElement>("#status")!;
const savedAt = document.querySelector<HTMLElement>("#saved-at")!;
const customQuestions =
  document.querySelector<HTMLDivElement>("#custom-questions")!;
const addQuestionButton =
  document.querySelector<HTMLButtonElement>("#add-question")!;
...
function readCustomQuestions(): {
  ...
  for (const row of customQuestions.querySelectorAll(
    "[data-custom-question]",
  )) {
    const question = row
      .querySelector<HTMLInputElement>(".custom-question-text")!
      .value.trim();
    const answer = row
      .querySelector<HTMLInputElement>(".custom-question-answer")!
      .value.trim();
...
if (stored.personalInfo.linkedIn)
  linkedinInput.value = stored.personalInfo.linkedIn;
if (stored.personalInfo.willingToRelocate)
  willingToRelocateInput.value = stored.personalInfo.willingToRelocate;
if (stored.personalInfo.howDidYouHear)
  howDidYouHearInput.value = stored.personalInfo.howDidYouHear;
...
if (missing.length > 0) {
  status.textContent = `Missing required: ${missing.join(", ")}. Fill them below and save to unlock autofill.`;
  status.style.color = "#f87171";
  clearTimeout(statusTimer);
  statusTimer = undefined;
  for (const field of missing)
    fieldElements[field]?.setAttribute("aria-invalid", "true");
}
